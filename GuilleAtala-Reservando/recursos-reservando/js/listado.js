...
Listado.prototype.buscarRestaurante = function(id) {
        ...
        var restauranteElegido = this.restaurantes.find(element => element.id === id)
        if (restauranteElegido == undefined) {
...
Listado.prototype.ObtenerCiudad = function() {
        ...
        var ciudades = this.restaurantes.map(elemento => elemento.ubicacion)
            //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
        return ListaSinRepetir(ciudades);
    }
...
Listado.prototype.ObtenerRubros = function() {
        ...
        var ListaRubros = this.restaurantes.map(elemento => elemento.rubro)
            //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
        return ListaSinRepetir(ListaRubros);
    }
...
Listado.prototype.ObtenerHorarios = function() {
        ...
        var arregloHorarios = this.restaurantes.map(elemento => elemento.horarios)
            //En este arreglo vamos a poner todos los horarios pero sin repetidos
        var arregloHora = [];
        arregloHorarios.forEach(element => {
            element.forEach(hora => arregloHora.push(hora))
        });
        return ListaSinRepetir(arregloHora);
    }
...
Listado.prototype.ObtenerRestaurantes = function(filtroRubro, filtroCiudad, filtroHorario) {
    ...
    ////funciones nuevas
    //crea un nuevo arreglo con los elementos sin repetir
function ListaSinRepetir(Repetidas) {
    var SinRepetir = Repetidas.filter(function(elem, index, self) {
        return index === self.indexOf(elem);
    });
    return SinRepetir.sort();
}

// nos devuelve un nuevo arreglo con los resultados de aplicarle la función que recibe por parámetro
Listado.prototype.mapear = function(arr1) {
    var algo = arr1,
        a = this.restaurantes.map(elemento => elemento.algo)
    return a
}
